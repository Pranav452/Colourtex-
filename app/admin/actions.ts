...
export async function addAllowedIp(formData: FormData): Promise<void> {
  ...
  await withRetry(() => sql`
    INSERT INTO colourtex_allowed_ips (ip, label) VALUES (${ip}, ${label})
...
export async function deleteAllowedIp(formData: FormData): Promise<void> {
  ...
  const rows = (await withRetry(() => sql`DELETE FROM colourtex_allowed_ips WHERE id = ${id} RETURNING ip`)) as { ip: string }[]

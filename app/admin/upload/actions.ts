...
export async function analyzeUpload(_prev: AnalyzeState, formData: FormData): Promise<AnalyzeState> {
  ...
  if (!(file instanceof File) || file.size === 0) {
    return { error: "Choose the Colourtex workbook first." }
  }
  ...
  try {
    ...
    const { shipments, report } = parseWorkbook(buffer)

    if (report.shipments === 0) {
      return { error: "No AWBs found in the file — check that it is the Colourtex workbook." }
    }

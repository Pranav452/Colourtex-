...
export async function login(_prev: LoginState, formData: FormData): Promise<LoginState> {
  ...
  if (verified.role === "client") {
    if (!(await ipAllowedForClient(ip))) {
      ...
      return { error: "This network is not authorised for the Colourtex portal. Contact LINKS." }
    }
    const device = await checkDevice(deviceId || undefined)
    if (!device.ok) {
      await audit("login-blocked-device", { user })
      return {
        error: "This device is not authorised for the Colourtex portal. Contact LINKS to register it.",
      }
    }

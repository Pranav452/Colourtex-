...
// Colourtex portal auth. Shares the Neon database with the VIPAR app but uses its
// own tables (colourtex_users, colourtex_devices, colourtex_allowed_ips, colourtex_access_log),
// its own cookie names and its own AUTH_SECRET so sessions cannot cross apps.

const FALLBACK_CLIENT_USER = process.env.COLOURTEX_USER ?? "colourtex"
const FALLBACK_CLIENT_PASS = process.env.COLOURTEX_PASS ?? "colourtex"
const FALLBACK_UPLOADER_USER = process.env.COLOURTEX_UPLOADER_USER ?? "upload"
const FALLBACK_UPLOADER_PASS = process.env.COLOURTEX_UPLOADER_PASS ?? "upload"
const FALLBACK_ADMIN_USER = process.env.COLOURTEX_ADMIN_USER ?? "admin"
const FALLBACK_ADMIN_PASS = process.env.COLOURTEX_ADMIN_PASS ?? "123456"
const AUTH_SECRET = process.env.AUTH_SECRET ?? "colourtex-links-dev-secret-change-me"
const MAX_DEVICES = Number(process.env.MAX_DEVICES ?? 3)
const SESSION_HOURS = Number(process.env.SESSION_HOURS ?? 24 * 7)

export const SESSION_COOKIE = "colourtex_session"
export const DEVICE_COOKIE = "colourtex_device"
...
// ---------------------------------------------------------------------------
// User verification — colourtex_users table, hardcoded fallback without DB
// ---------------------------------------------------------------------------
  ...
  if (sql) {
    try {
      const rows = (await withRetry(() => sql`
        SELECT password_hash, role FROM colourtex_users WHERE username = ${username}
...
export async function listAllowedIps(): Promise<AllowedIp[]> {
  const sql = getSql()
  if (!sql) return []
  return (await withRetry(() => sql`SELECT id, ip, label, created_at FROM colourtex_allowed_ips ORDER BY id`)) as AllowedIp[]
}
...
export async function listDevices(): Promise<DeviceRecord[]> {
  ...
  if (!sql) return []
  const rows = (await withRetry(() => sql`
    SELECT id, first_seen, ip, user_agent FROM colourtex_devices ORDER BY first_seen
...
export async function removeDevice(id: string): Promise<void> {
  const sql = getSql()
  if (!sql) return
  await withRetry(() => sql`DELETE FROM colourtex_devices WHERE id = ${id}`)
}
...
export async function checkDevice(existingDeviceId?: string): Promise<DeviceCheck> {
  ...
  const id = randomUUID()
  await withRetry(() => sql`
    INSERT INTO colourtex_devices (id, first_seen, ip, user_agent)
...
export async function audit(event: string, extra: Record<string, string> = {}): Promise<void> {
  try {
    ...
    await withRetry(() => sql`
      INSERT INTO colourtex_access_log (event, username, ip, user_agent, meta)
...
export async function recentAuditRows(limit = 30): Promise<AuditRow[]> {
  ...
  return (await withRetry(() => sql`
    ...
    FROM colourtex_access_log ORDER BY id DESC LIMIT ${limit}
  `)) as AuditRow[]
}

  ...
  if (sql) {
    try {
      const rows = (await withRetry(() => sql`
        ...
        FROM colourtex_dataset_versions WHERE active = true
...
export async function saveDataset(
  ...
  const chargeable = Math.round(shipments.reduce((a, s) => a + s.chargeableWt, 0))
...
export async function listVersions(): Promise<DatasetVersion[]> {
  ...
  const rows = (await withRetry(() => sql`
    ...
    FROM colourtex_dataset_versions ORDER BY uploaded_at DESC
...
export async function activateVersion(id: number): Promise<void> {
  ...
  await withRetry(() =>
    sql.transaction([
      sql`UPDATE colourtex_dataset_versions SET active = false WHERE active = true`,
      sql`UPDATE colourtex_dataset_versions SET active = true WHERE id = ${id}`,
    ]),
  )
}

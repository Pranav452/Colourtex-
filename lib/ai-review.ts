...
function rowContext(shipments: Shipment[], awb: string | null): string {
  ...
  return JSON.stringify({
    ...
    flightDetails: s.flightDetailsRaw ?? s.legs.map((l) => `${l.carrier}${l.flightNo}-${l.to}${l.status ? `-${l.status}` : ""}`).join(" "),
    clearanceDate: s.clearanceDate,
  })
}

const BATCH_SYSTEM = `You are a data-quality analyst for an air-freight forwarder's AWB spreadsheet (Colourtex dye exports out of Mumbai). You receive parser warnings raised while ingesting the ops sheet, each with the parsed row for context. For every warning explain in plain English (a) the likely root cause in the spreadsheet and (b) the exact fix the uploader should make in the sheet or in the preview editor. Be terse and concrete — name the cell/value to change. Respond with JSON: {"items":[{"index":<number>,"cause":"...","fix":"..."}]}`
